...
export function AddToCart({ 
  ...
  const [localQuantity, setLocalQuantity] = useState<string | null>(null);
  const displayQuantity = localQuantity ?? quantity.toString();

  // Используем ref для отслеживания изменений initialQuantity, чтобы избежать зацикливания
  const prevInitialQuantity = useRef<number | undefined>(undefined);

  useEffect(() => {
    // Синхронизируем только если initialQuantity действительно изменился и это панель в корзине
    if (initialQuantity !== undefined && initialQuantity !== prevInitialQuantity.current) {
      prevInitialQuantity.current = initialQuantity;
      
      if (isPanel && quantity > 0 && quantity !== initialQuantity) {
        // Используем setTimeout чтобы вынести обновление из текущего цикла рендеринга
        // Это часто помогает избежать "Maximum update depth exceeded"
  ...
  }, [initialQuantity, itemId, isPanel]); // Убрали quantity из зависимостей
  ...
  const handleAdd = () => {
    ...
    if (extraItems && extraItems.length > 0) {
      extraItems.forEach(item => {
        // Если это набор (например, монтажный комплект), добавляем его составляющие
        if (item.product.bundleItems && item.product.bundleItems.length > 0) {
          item.product.bundleItems.forEach(kitItem => {
            const fakeProduct: Product = {
              id: kitItem.id,
              name: kitItem.name,
              price: kitItem.price,
              slug: kitItem.slug || "",
              images: [kitItem.image || ""],
              category: "accessories",
              description: "",
              specifications: { material: "", width: 0, height: 0, depth: 0 },
              colors: [],
            ...
            addToCart(fakeProduct, kitItem.quantity * item.quantity);
          });
        } else {
          // Иначе добавляем как обычный товар
  ...
  return (
    <div className="relative group/tooltip space-y-4">
      <Button 
        onClick={handleAdd}
        disabled={!inStock}
        className={`h-14 px-8 text-[11px] font-bold uppercase tracking-[0.2em] transition-all duration-500 w-full rounded-none ${
          !inStock
